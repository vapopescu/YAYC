...
//make request for video data
  function retrieve_videos(userData) {
    data = apiRequest("GET", null, 'user/' + userData.id + '?load=subscriptions.target.videos', userData.token); //retrieve data for videos
    if (data != null) {
      console.log("Videos retrieved succesfully!! generate cards");
      construct_card_videos(data.response.subscriptions); //create the card_video objects
    } else {
...
//create the card_video objects
  function construct_card_videos(data) {
    ...
    for (i = 0; i < data.length; i++)
      ...
      card_author_id = data[i].target.videos[j].user_id;
      card_text = data[i].target.videos[j].description;
      card_created_date = data[i].target.videos[j].created_at;
      card_views = data[i].target.videos[j].view_count;
      card_likes = data[i].target.videos[j].likes;
